...
class AuthManager {
    ...
    static async login(email, password) {
        try {
            // 🚨 FAILLE DE SÉCURITÉ : Parse les payloads JSON pour NoSQL injection
            let emailPayload = email;
            let passwordPayload = password;

            // Si l'input ressemble à du JSON, on le parse pour l'injection NoSQL
            if (typeof email === 'string' && email.trim().startsWith('{')) {
                try {
                    emailPayload = JSON.parse(email);
                    console.log('🚨 EMAIL PAYLOAD PARSED:', emailPayload);
                } catch (e) {
                    console.log('Invalid JSON in email, using as string');
                }
            }

            if (typeof password === 'string' && password.trim().startsWith('{')) {
                try {
                    passwordPayload = JSON.parse(password);
                    console.log('🚨 PASSWORD PAYLOAD PARSED:', passwordPayload);
                } catch (e) {
                    console.log('Invalid JSON in password, using as string');
                }
            }
